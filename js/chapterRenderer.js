// js/chapterRenderer.js (重构版 - 更新导入路径和 YouTube URL)
 ...
 * @param {Function} navigateToChapterCallback - 章节导航回调函数。
...
export function renderSingleChapterContent(chapterContent, currentChapterTooltips, wordFrequenciesMap, maxFreq, navigateToChapterCallback, subtitleData = []) {
    ...
    chapterContent.paragraphs.forEach(item => {
        ...
        if (typeof item === 'string') {
            ...
            const videoId = extractVideoId(item.video); // 使用 youtube.js 的 extractVideoId

            // 修正：使用 youtube.js 的 getYouTubeEmbedUrl 来生成正确的 src
            iframe.src = videoId ? getYouTubeEmbedUrl(videoId, true) : ''; // 启用 JS API

            if (iframe.src) { // 只有当 src 有效时才添加到 DOM
